...
initialState robot goal blocks
    ...
    | any (not . isValidCoord) blocks = errorState
  ...
  where
    ...
    hasOverlap = length allCoords /= length (nub allCoords)
...
isValidMove (robot, goal, blocks) dir
    ...
    | not (isValidCoord nextRobot) = False
    ...
    | nextRobot == goal || nextRobot `elem` blocks =
        ...
        isValidCoord behindBox && behindBox /= goal && not (behindBox `elem` blocks)
  ...
  where
    ...
    nextRobot = moveCoord robot dir     -- La casilla a la que el robot quiere avanzar.
    behindBox = moveCoord nextRobot dir -- La casilla a donde se desplazaría la caja empujada. 
...
applyMove (robot, goal, blocks) dir = (nextRobot, nextGoal, nextBlocks)
  where
    -- El robot se desplaza obligatoriamente a su siguiente casilla calculada.
    nextRobot = moveCoord robot dir
